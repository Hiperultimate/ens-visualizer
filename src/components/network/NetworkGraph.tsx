...
export const NetworkGraph = forwardRef<NetworkGraphRef, NetworkGraphProps>(
  ...
  const loadGraphData = useCallback(async () => {
    ...
    try {
      ...
      if (data.nodes && data.connections) {
        ...
        const flowEdges: Edge[] = data.connections.map(
  ...
  const handleDeleteNode = useCallback(
    async (nodeId: string) => {
      try {
        const response = await fetch(
          `/api/nodes?nodeId=${nodeId}&userId=${userId}`,
          { method: 'DELETE' }
        )
        ...
        if (response.ok) {
          ...
          const error = await response.json()
          console.error('Failed to delete node:', error.error)
        }
      } catch (error) {
  ...
  return (
    <div className="w-full h-full relative" style={{ minHeight: '400px' }}>
      <ReactFlow
        ...
        // Allow multiple connections from/to the same node (many-to-many)
